...
/****************************************************************************
 * <b>Title</b>: AirlineTicketCalculator.java <b>Project</b>: SMT-Kata
 * <b>Description: </b> Airline Ticket Calculator Kata You are given a huge list
 * of airline ticket prices between different cities around the world on a given
 ...
 * 
 * { {'JFK', 'ATL', 150}, {'ATL', 'SFO', 400}, {'ORD', 'LAX', 200}, {'LAX',
 * 'DFW', 80}, {'JFK', 'HKG', 800}, {'ATL', 'ORD', 90}, {'JFK', 'LAX', 500}, }
 * 
...
public class AirlineTicketCalculator {
	...
	public int findCheapest(List<Ticket> flights, String source, String dest, int maxConnections) {
		if (flights == null || flights.isEmpty() || maxConnections < 1 || dest == null || source == null
				|| dest.length() == 0 || source.length() == 0) {
			return 0;
		}

		// recursion to move through all possible routes and find minPrice
		// variables needed: connection count, cheapest price, and destination->source,
		// source, maxConnections,
		findConnections(flights, source, dest, 0, 0, maxConnections);
	...
	public void findConnections(List<Ticket> flights, String source, String dest, int price, int leg, int maxCon) {
		...
		for (Ticket t : flights) {
			if (t == null)
				continue;
			if (t.sourceCity.equals(source)) {
				findConnections(flights, t.destCity, dest, price + t.cost, leg + 1, maxCon);
			}
		}
	...
	public class Ticket {
		 ...
		 * @param cost       Cost of the ticket to fly form the source to the
		 *                   destination
		 */

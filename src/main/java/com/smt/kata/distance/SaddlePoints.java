...
/****************************************************************************
 * <b>Title:</b> SaddlePoints.java <b>Project:</b> SMT-Kata <b>Description:</b>
 * Saddle Points Detect saddle points in a matrix.
 * 
 * So say you have a matrix:
 * 
 * 1 2 3 |--------- 1 | 9 8 7 2 | 5 3 2 <--- saddle point at column 1, row 2,
 * with value 5 3 | 6 6 7
 * 
...
public class SaddlePoints {
	...
	public List<int[]> getCoordinates() {
		...
		for (int i = 0; i < matrix.length; i++) {
			...
			for (int j = 0; j < matrix[i].length; j++) {
				...
				if (isColLow(j, matrix[i][j]) && isRowHigh(i, matrix[i][j])) {
					int rowHigh = i;
					int colLow = j;
					int value = matrix[i][j];
					coords.add(new int[] { colLow, rowHigh, value });
					System.out.println(colLow + ", " + rowHigh + ", " + value);
				}
	...
	public boolean isColLow(int col, int value) {
		for (int i = 0; i < matrix.length; i++) {
	...
	public boolean isRowHigh(int row, int value) {
